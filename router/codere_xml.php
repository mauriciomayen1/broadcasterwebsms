...
$logger   = new Zend_Log();
$filename = date('Ymd');
$filename = '/home/codere.cm-operations.com/pub_html/dashboard/logs/envioCode_prueba_'.$filename.'.log';
...
$logger->log ( "POST::: ". print_r($post, true), Zend_Log::INFO );

$xml = simplexml_load_string($post);
$xml->registerXPathNamespace('ns1', 'http://www.csapi.org/schema/parlayx/sms/notification/v2_2/local');
foreach ($xml->xpath('//ns1:message') as $item)
{
  $logger->log ( "ITEMS::: ". print_r((string)$item->message, true), Zend_Log::INFO );
}
...
$campania = (string)$xml["campaign"];
$timestamp = (string)$xml["timestamp"];

#######VERIFICAMOS SI EXISTE UNA PETICION

$query_ip = "SELECT * FROM bloqueo WHERE campania = '$campania' AND timestamp = '$timestamp' AND status = '1'";
$result_ip = $db->fetchAll($query_ip);

if (!empty($result_ip)) {
	######SI EXISTE, TERMINAMOS LA PETICION
	$logger->log ( ":::EXISTE UNA PETICION ACTIVA:::", Zend_Log::INFO );
	exit;
} else {
	#####SI NO, INSERTAMOS IP PARA EVITAR PETICIONES CONSECUTIVAS
	$data_bloqueo = array(
		'campania'       => (string)$xml["campaign"],
		'timestamp'      => (string)$xml["timestamp"],
	...
	$logger->log('Array data_bloqueo:: '.print_r($data_bloqueo,true), Zend_Log::INFO);
	$insert_bloqueo = $db->insert('bloqueo',$data_bloqueo);
	$logger->log('SE INSERTO:: '.print_r($insert_bloqueo,true), Zend_Log::INFO); 
}
			...
			if($longitud_msisdn < 10){
				...
				if($bandera == 1){
					if($carrier == 'Telcel'){
						...
						$url = 'http://localhost:13013/cgi-bin/sendsms?username=sendsmsmt_41414&password=kaiser&to=%s&text=%s';
					...
					elseif($carrier == 'Movistar' || $carrier== 'Virgin'){
						...
						$msisdn2 = "52".$msisdn;
						/*$url = "http://administrador.cm-operations.com/movistar/router/router_mt.php?SOA=$msisdn&dial=26262&message=$contenido&service=CODEREXML&username=sendsmsmovistar_26262";*/
...
///ACTUALIZAMOS IP PARA RECIBIR SIGUIENTE PETICION

$data_update = array(
    'status'      => 0,
);
$logger->log ( "data_update::: ". print_r($data_update, true), Zend_Log::INFO );
$result_update = $db->update('bloqueo', $data_update, "campania = '$campania' AND timestamp = '$timestamp' AND status = '1'");
$logger->log ( "Result UPDATE::: ". print_r($result_update, true), Zend_Log::INFO );

$fecharesponse = date('Y-m-d H:i:s');
$response = '<?xml version="1.0" encoding="UTF-8"?><replyBatch timestamp="'.$fecharesponse.'" total="'.$i.'">'. $resultb ."</replyBatch>
";
