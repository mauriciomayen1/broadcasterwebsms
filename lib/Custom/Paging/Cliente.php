...
class Ns_Paging_Cliente extends Ns_Paging
{
	public $db;
	public $select;
	public $smarty;
	...
	public function setTemplate($template) {
		...
		$archivo = new Ns_QueryTool($config->application['db'][0]['dburl'], 'archivo');

		foreach ($pagerData['items'] as $k => $v) {
			foreach ($v as $k2 => $v2) {
				$pagerDataStrip['items'][$k][$k2] = stripslashes($v2);
				// ----
				if ($k2 =='clientedocumentacion_cartaencomienda') {
					$archivo->reset();
					$pagerDataStrip['items'][$k][$k2] = $archivo->get($v2); 
				}	

				if ($k2 =='clientedocumentacion_cedula') {
					$archivo->reset();
					$pagerDataStrip['items'][$k][$k2] = $archivo->get($v2); 
				}	

				if ($k2 =='clientedocumentacion_comprobantedomicilio') {
					$archivo->reset();
					$pagerDataStrip['items'][$k][$k2] = $archivo->get($v2); 
				}	

				if ($k2 =='clientedocumentacion_actaconstitutiva') {
					$archivo->reset();
					$pagerDataStrip['items'][$k][$k2] = $archivo->get($v2); 
				}	

				if ($k2 =='clientedocumentacion_r2') {
					$archivo->reset();
					$pagerDataStrip['items'][$k][$k2] = $archivo->get($v2); 
				}	

				if ($k2 =='clientedocumentacion_solicitudcredito') {
					$archivo->reset();
					$pagerDataStrip['items'][$k][$k2] = $archivo->get($v2); 
				}	

				if ($k2 =='clientedocumentacion_identificacion') {
					$archivo->reset();
					$pagerDataStrip['items'][$k][$k2] = $archivo->get($v2); 
				}	
				//----
